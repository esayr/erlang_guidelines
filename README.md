...
建议先阅读官方规范: http://www.erlang.se/doc/programming_rules.shtml

***

目录:
* [联系我们](#Contact-Us)
* [约定 & 规则](#conventions--rules)
  * [源代码样式](#source-code-layout)
    * [使用空格替换 tab](#spaces-over-tabs)
    * [在 操作符前后 和 逗号后 加空格](#use-your-spacebar)
    * [行尾不要有空格或空字节](#no-trailing-whitespace)
    * [单行代码控制在80字以内](#80-column-per-line)
    * [维持老代码的风格](#maintain-existing-style)
    * [避免深层嵌套](#avoid-deep-nesting)
    * [写一个小的函数替换 case 表达式](#more-smaller-functions-over-case-expressions)
    * [按逻辑整理函数代码块 私有/导出函数分开](#group-functions-logically)
    * [所有的 -type 集合放在文件头部](#get-your-types-together)
    * [不使用"神"模块](#no-god-modules)
    * [简化单元测试](#simple-unit-tests)
    * [使用函数或变量替换出现在多处的相同代码](#honor-dry)
    * [避免动态调用](#avoid-dynamic-calls)
    * [把功能类似的模块文件整理到一个目录中](#group-modules-in-subdirectories-by-functionality)
    * [不要写"套管"程序](#dont-write-spaghetti-code)
  * [语法](#syntax)
    * [避免使用 if 表达式](#avoid-if-expressions)
    * [避免嵌套使用 try...catches](#avoid-nested-try-catches)
  * [命名](#naming)
    * [命名要保持一致性](#be-consistent-when-naming-concepts)
    * [总是为OTP 模块写一条 state 记录](#explicit-state-should-be-explicitly-named)
    * [不要使用 _Ignored 变量](#dont-use-_ignored-variables)
    * [避免在参数中使用布尔值](#avoid-boolean-parameters)
    * [为相同模块统一约定一个命名规则](#stick-to-one-convention-for-naming-modules)
    * [原子命名 不允许存在大写字母](#lowercase-atoms)
    * [函数命名 小写字母+下划线分隔符](#function-names)
    * [变量命名 首字母大写+驼峰 不允许有下划线](#variable-names)
  * [字符串](#strings)
    * [使用 IOLists 替换字符串连接符++](#iolists-over-string-concatenation)
  * [宏](#macros)
    * [尽量不要使用宏](#no-macros)
    * [宏名字要求全大写+下划线分隔符](#uppercase-macros)
    * [不要为模板或函数定义为一个宏](#no-module-or-function-name-macros)
  + [记录](#records)
    * [记录命名 记录和记录里的字段名都必须是小写字母+下划线分隔符方式](#record-names)
    * [-record 代码放在函数代码块之前](#records-go-first)
    * [不要在多个模板中共享一个记录](#dont-share-your-records)
    * [避免在 -spec 中出现记录](#avoid-records-in-specs)
    * [总是为记录的字段定义类型](#types-in-records)
  * [其它](#misc)
    * [为所有可导出的函数写 -spec](#write-function-specs)
    * [使用 -callback 属性替换 behaviour_info/1](use--callback-attributes-over-behaviour_info1)
    * [不要在 hrl 文件中使用 -include](#no-nested-header-inclusion)
    * [不要在 hrl 文件中使用 -type](#no-types-in-include-files)
    * [避免使用 -import](#dont-import)
  * [工具](#tools)
    * [指定依赖包的版本号](#lock-your-dependencies)
    * [让错误暴露出来](#loud-errors)
    * [为错误指定适当的级别](#properly-use-logging-levels)
    * [依赖包地址使用 git 协议](#prefer-the-git-protocol-over-others-when-specifying-dependency-urls)
* [建议 & 最佳实践](#suggestions--great-ideas)
  * [变量使用驼峰替换首字母大小+下划线命名法](#camelcase-over-under_score)
  * [在可理解的情况下尽可能简化变量名](#prefer-shorter-but-still-meaningful-variable-names)
  * [注释分级别](#comment-levels)
  * [保持函数小而简洁](#keep-functions-small)
  * [使用 -behaviour](#use-behaviours)
  * [防守式编程](#when-programming-defensively-do-so-on-client-side)
  * [避免不必要的调用 length/1](#avoid-unnecesary-calls-to-length1)
  * [按功能把可完全抽离的代码做成一个 application](#move-stuff-to-independent-applications)
  * [使用外观模式来设计类库](#use-the-facade-pattern-on-libraries)
  * [为导出的函数指定 -type](#types-in-exported-functions)
